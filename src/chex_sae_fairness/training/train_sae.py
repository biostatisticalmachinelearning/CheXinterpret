...
def train_sae_model(
    ...
    model = SparseAutoencoder(input_dim=train_features.shape[1], latent_dim=cfg.latent_dim).to(device)
    ...
    for epoch in range(cfg.epochs):
        model.train()
        train_running = {"total": 0.0, "mse": 0.0, "sparsity": 0.0, "steps": 0}

        for (batch,) in train_loader:
            batch = batch.to(device)
            x_hat, z = model(batch)
            loss = sae_loss(batch, x_hat, z, cfg.l1_lambda)
        ...
        train_curve.append(_normalize_running(epoch, train_running))

        if (epoch + 1) % cfg.eval_every_epochs == 0 or epoch == cfg.epochs - 1:
            valid_curve.append(_evaluate_epoch(model, valid_loader, cfg.l1_lambda, epoch, device))
...
def _evaluate_epoch(
    ...
    model.eval()
    running = {"total": 0.0, "mse": 0.0, "sparsity": 0.0, "steps": 0}

    with torch.no_grad():
        for (batch,) in loader:
            batch = batch.to(device)
            x_hat, z = model(batch)
            loss = sae_loss(batch, x_hat, z, l1_lambda)
    ...
    return _normalize_running(epoch, running)


def _normalize_running(epoch: int, running: dict[str, float]) -> dict[str, float]:
