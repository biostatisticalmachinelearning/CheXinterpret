...
def run_study_cli() -> None:
    ...
    parser.add_argument(
        "--sweep-config",
        default="configs/sae_sweep.yaml",
        help="Path to SAE sweep YAML. If missing, a default sweep is autogenerated.",
    )
    parser.add_argument(
        "--single-sae",
        action="store_true",
        help="Run only a single SAE study (disable multi-SAE sweep orchestration).",
    )
    parser.add_argument(
        "--force-features",
        action="store_true",
        help="Recompute CheXagent embeddings even if feature cache exists.",
    )
    parser.add_argument(
        "--run-name",
        default=None,
        help="Optional name for the timestamped run folder.",
    )
    _add_logging_args(parser)
    args = parser.parse_args()
    ...
    if args.single_sae:
        ...
        logger.info("Single-SAE study finished.")
    ...
    run_dir = create_timestamped_run_dir(cfg.output_root, run_name=args.run_name)
    log_path = Path(args.log_file).expanduser() if args.log_file else run_dir / "logs" / "run_study.log"
    configure_logging(level=args.log_level, log_file=log_path)
    logger.info("Starting comprehensive multi-SAE pipeline.")
    logger.info("Run root: %s", run_dir.resolve())
    ...
    result = run_comprehensive_study(
    ...
    logger.info("Comprehensive study finished.")
    logger.info("Summary: %s", Path(result["run_root"]) / "run_summary.json")
    logger.info("Best SAE run: %s", result["best_run_name"])
    logger.info("Best report: %s", result["best_study_report"])
